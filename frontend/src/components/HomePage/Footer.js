...
function Footer({ isLoaded }){
  ...
  function submitContactForm(e){
    ...
    dispatch(submitForm(submitValue))
  }
  return (
    <div id='footer' className='homepage__footer'>
        <form onSubmit={(e) => submitContactForm(e)} className={"footer__contact__us scroll-transition-fade " +(useOnScreen(ref) ? 'nothing' : 'below-viewport')}  ref={ref}>
            ...
            <input onChange={(e) => setEmail(e.target.value)} className='contact__email' required type='email' placeholder='Email'/>
            <textarea onChange={(e) => setMessage(e.target.value)} className='contact__message' required placeholder='Message'/>
